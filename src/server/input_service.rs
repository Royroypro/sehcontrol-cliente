...
impl VirtualInputState {
    fn new() -> Option<Self> {
        VirtualInput::new(
            ...
            // 1. Press "Command" key in Sehcontrol
            // 2. Move the physical mouse
            // 3. Press "V" key in Sehcontrol
            ...
            // we consider only Sehcontrol operates the controlled side.
            //
...
fn key_sleep() {
    ...
    // Though `/Applications/Sehcontrol.app/Contents/MacOS/sehcontrol --server` in terminal is ok.
    let now = Instant::now();
...
pub async fn lock_screen() {
    ...
    if #[cfg(target_os = "linux")] {
        ...
        // loginctl lock-session also not work, they both work run sehcontrol from cmd
        std::thread::spawn(|| {
