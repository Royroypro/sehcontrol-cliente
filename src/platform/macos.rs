...
// remove just one app from all the permissions: tccutil reset All com.carriez.sehcontrol
...
fn correct_app_name(s: &str) -> String {
    ...
    if let Some(bundleid) = get_bundle_id() {
        s = s.replace("com.carriez.sehcontrol", &bundleid);
    }
    s = s.replace("sehcontrol", &crate::get_app_name().to_lowercase());
    s = s.replace("Sehcontrol", &crate::get_app_name());
    s
}
...
pub fn start_os_service() {
    ...
    /* // mouse/keyboard works in prelogin now with launchctl asuser.
       ...
       // one way is to start with `launchctl asuser <uid> open -n -a /Applications/Sehcontrol.app/ --args --cm`,
       // this way --cm is started with the user privilege, but we will have problem to start another Sehcontrol.app
...
pub fn update_me() -> ResultType<()> {
    ...
    let cmd = std::env::current_exe()?;
    // Sehcontrol.app/Contents/MacOS/Sehcontrol
    let app_dir = cmd
        .parent()
        .and_then(|p| p.parent())
        .and_then(|p| p.parent())
