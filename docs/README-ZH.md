...
> [!CAUTION]
> **免责声明:** <br>
> Sehcontrol 的开发人员不纵容或支持任何不道德或非法的软件使用行为。滥用行为，例如未经授权的访问、控制或侵犯隐私，严格违反我们的准则。作者对应用程序的任何滥用行为概不负责。
...
[![Sehcontrol Server Pro](https://img.shields.io/badge/RustDesk%20Server%20Pro-%E9%AB%98%E7%BA%A7%E5%8A%9F%E8%83%BD-blue)](https://rustdesk.com/pricing.html)
...
Sehcontrol 期待各位的贡献. 如何参与开发? 详情请看 [CONTRIBUTING-ZH.md](CONTRIBUTING-ZH.md).
...
cd sehcontrol # 进入文件夹
docker build -t "sehcontrol-builder" . # 构建容器
```
...
* 针对国内网络访问问题，可以做以下几点优化：  
   ...
   4. docker build 命令后面加上 proxy 参数
      ...
      ```
      docker build -t "sehcontrol-builder" . --build-arg http_proxy=http://host:port --build-arg https_proxy=http://host:port
      ```
...
生成的可执行程序在 target 目录下，可直接通过指令运行调试 (Debug) 版本的 Sehcontrol:
```sh
...
请注意：
* 请保证您运行的目录是在 Sehcontrol 库的根目录内，否则软件会读不到文件。
