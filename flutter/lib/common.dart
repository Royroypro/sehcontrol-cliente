...
List<String>? urlLinkToCmdArgs(Uri uri) {
  ...
  if (uri.authority.isEmpty &&
    ...
    // sehcontrol://<connect-id>
    // sehcontrol://<connect-id>/r
    // sehcontrol://<connect-id>/r@<server>
    command = '--connect';
    id = uri.authority;
    if (uri.path.length > 1) {
      id = id + uri.path;
...
Future<void> onActiveWindowChanged() async {
  ...
  if (rustDeskWinManager.getActiveWindows().isEmpty) {
    ...
    try {
      ...
      debugPrint("Start closing Sehcontrol...");
      ...
      if (isMacOS) {
        ...
        // 2024-11-11 11:41:11.546 Sehcontrol[90272:2567686] Failed to create a FlutterPlatformMessageResponseHandle (2)
        // embedder.cc (2672): 'FlutterEngineSendPlatformMessage' returned 'kInvalidArguments'. Invalid engine handle.
        // 2024-11-11 11:41:11.565 Sehcontrol[90272:2567686] Failed to send message to Flutter engine on channel 'flutter/lifecycle' (2).
        // ```
        periodic_immediate(
...
class ServerConfig {
  ...
  /// decode from shared string (from user shared or sehcontrol-server generated)
  /// also see [encode]
...
const kMacOSPermChannel = MethodChannel("org.sehcontrol.sehcontrol/host");
