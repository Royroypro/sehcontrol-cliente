...
{
  HINSTANCE hInstance = LoadLibraryA("sehcontrol.dll");
  if (!hInstance)
  {
    std::cout << "Failed to load sehcontrol.dll." << std::endl;
    return EXIT_FAILURE;
  }
  ...
  if (!c_args)
  {
    ...
    // std::cout << "Sehcontrol [" << args_str << "], core returns false, exiting without launching Flutter app." << std::endl;
    return EXIT_SUCCESS;
  }
  ...
  std::wstring app_name = L"Sehcontrol";
