...
hbb_name = 'sehcontrol' + ('.exe' if windows else '')
...
def generate_control_file(version):
    ...
    content = """Package: sehcontrol
Section: net
Priority: optional
Version: %s
Architecture: %s
Maintainer: sehcontrol <info@sehcontrol.com>
...
DRM_PACKAGE_NAME = 'sehcontrol-unattended-wayland'
...
def stage_libdrmtap_into_deb(so_path):
    ...
    # symlink is needed: libdrmtap is resolved by ABSOLUTE path (/usr/lib/sehcontrol/libdrmtap.so.0) at
    # the in-process dlopen site (drmtap_dl.rs), so the deb does NOT drop /usr/lib/sehcontrol into the
    ...
    system2('mkdir -p tmpdeb/usr/lib/sehcontrol')
    ...
    system2(f'cp "{so_path}" tmpdeb/usr/lib/sehcontrol/')
    system2(f'ln -sf "{so_basename}" tmpdeb/usr/lib/sehcontrol/libdrmtap.so.0')
...
def measured_glibc_floor():
    ...
    paths = [p for p in glob.glob('tmpdeb/usr/lib/sehcontrol/libdrmtap.so.0.*')
             + glob.glob('tmpdeb/usr/share/sehcontrol/lib/sehcontrol.so')
             + glob.glob('tmpdeb/usr/share/sehcontrol/sehcontrol')
...
def retarget_control_to_drm_variant():
    ...
    out = []
    for line in lines:
        if line.startswith('Package: sehcontrol'):
            out.append(f'Package: {DRM_PACKAGE_NAME}\n')
            out.append('Conflicts: sehcontrol\nReplaces: sehcontrol\nProvides: sehcontrol\n')
...
def build_flutter_deb(version, features):
    ...
    system2('mkdir -p tmpdeb/usr/share/sehcontrol')
    system2('mkdir -p tmpdeb/etc/sehcontrol/')
    system2('mkdir -p tmpdeb/etc/pam.d/')
    system2('mkdir -p tmpdeb/usr/share/sehcontrol/files/systemd/')
    system2('mkdir -p tmpdeb/usr/share/icons/hicolor/256x256/apps/')
    system2('mkdir -p tmpdeb/usr/share/icons/hicolor/scalable/apps/')
    system2('mkdir -p tmpdeb/usr/share/applications/')
    system2('mkdir -p tmpdeb/usr/share/polkit-1/actions')
    system2('rm tmpdeb/usr/bin/sehcontrol || true')
    system2(
        f'cp -r {flutter_build_dir}/* tmpdeb/usr/share/sehcontrol/')
    system2(
        'cp ../res/sehcontrol.service tmpdeb/usr/share/sehcontrol/files/systemd/')
    system2(
        'cp ../res/128x128@2x.png tmpdeb/usr/share/icons/hicolor/256x256/apps/sehcontrol.png')
    system2(
        'cp ../res/scalable.svg tmpdeb/usr/share/icons/hicolor/scalable/apps/sehcontrol.svg')
    system2(
        'cp ../res/sehcontrol.desktop tmpdeb/usr/share/applications/sehcontrol.desktop')
    system2(
        'cp ../res/sehcontrol-link.desktop tmpdeb/usr/share/applications/sehcontrol-link.desktop')
    system2(
        'cp ../res/startwm.sh tmpdeb/etc/sehcontrol/')
    system2(
        'cp ../res/xorg.conf tmpdeb/etc/sehcontrol/')
    system2(
        'cp ../res/pam.d/sehcontrol.debian tmpdeb/etc/pam.d/sehcontrol')
    system2(
        "echo \"#!/bin/sh\" >> tmpdeb/usr/share/sehcontrol/files/polkit && chmod a+x tmpdeb/usr/share/sehcontrol/files/polkit")
    ...
    system2('dpkg-deb -b tmpdeb sehcontrol.deb;')

    system2('/bin/rm -rf tmpdeb/')
    system2('/bin/rm -rf ../res/DEBIAN/control')
    os.rename('sehcontrol.deb', '../sehcontrol-%s.deb' % version)
    if ships_so:
        # Named apart from the stock package so installing the consent-free variant is a deliberate act.
        os.rename('../sehcontrol-%s.deb' % version, f'../{DRM_PACKAGE_NAME}-{version}.deb')
    os.chdir("..")


DRMTAP_DLOPEN_MARKER = b'/usr/lib/sehcontrol/libdrmtap.so.0'
...
def assert_staged_binary_is_drm():
    ...
    """
    binaries = [p for p in glob.glob('tmpdeb/usr/share/sehcontrol/lib/sehcontrol.so')
                + glob.glob('tmpdeb/usr/share/sehcontrol/sehcontrol') if os.path.isfile(p)]
...
def build_deb_from_folder(version, binary_folder, want_drm=False):
    ...
    system2('rm tmpdeb/usr/bin/sehcontrol || true')
    system2(
        f'cp -r ../{binary_folder}/* tmpdeb/usr/share/sehcontrol/')
    system2(
        'cp ../res/sehcontrol.service tmpdeb/usr/share/sehcontrol/files/systemd/')
    system2(
        'cp ../res/128x128@2x.png tmpdeb/usr/share/icons/hicolor/256x256/apps/sehcontrol.png')
    ...
    bundled_glob = glob.glob('tmpdeb/usr/share/sehcontrol/libdrmtap.so.0.*')
    ...
    if want_drm:
        ...
        if bundle_carries_so:
            ...
            system2(f'rm -f "{so}"')
            system2('rm -f tmpdeb/usr/share/sehcontrol/libdrmtap.so tmpdeb/usr/share/sehcontrol/libdrmtap.so.0')
        else:
...
def build_flutter_dmg(version, features):
    ...
    # copy dylib
    system2(
        "cp target/release/libsehcontrol.dylib target/release/sehcontrol.dylib")
    os.chdir('flutter')
    ...
    system2('cp -rf ../target/release/service ./build/macos/Build/Products/Release/Sehcontrol.app/Contents/MacOS/')
    '''
    system2(
        "create-dmg --volname \"Sehcontrol Installer\" --window-pos 200 120 --window-size 800 400 --icon-size 100 --app-drop-link 600 185 --icon Sehcontrol.app 200 190 --hide-extension Sehcontrol.app sehcontrol.dmg ./build/macos/Build/Products/Release/Sehcontrol.app")
    os.rename("sehcontrol.dmg", f"../sehcontrol-{version}.dmg")
    '''
    os.chdir("..")
...
def build_flutter_arch_manjaro(version, features):
    ...
    os.chdir('flutter')
    system2('flutter build linux --release')
    system2(f'strip {flutter_build_dir}/lib/sehcontrol.so')
    os.chdir('../res')
...
def build_flutter_windows(version, features, skip_portable_pack):
    if not skip_cargo:
        system2(f'cargo build --locked --features {features} --lib --release')
        if not os.path.exists("target/release/sehcontrol.dll"):
...
def main():
    ...
    if windows:
        ...
        # system2('upx.exe target/release/sehcontrol.exe')
        system2('mv target/release/sehcontrol.exe target/release/Sehcontrol.exe')
        pa = os.environ.get('P')
        if pa:
            # https://certera.com/kb/tutorial-guide-for-safenet-authentication-client-for-code-signing/
            system2(
                f'signtool sign /a /v /p {pa} /debug /f .\\cert.pfx /t http://timestamp.digicert.com  '
                'target\\release\\sehcontrol.exe')
        else:
            print('Not signed')
        os.makedirs(res_dir, exist_ok=True)
        system2(
        ...
        system2('python -m pip install -r requirements.txt')
        system2(
            f'python ./generate.py -f ../../{res_dir} -o . -e ../../{res_dir}/sehcontrol-{version}-win7-install.exe')
        system2(f'mv ../../{res_dir}/sehcontrol-{version}-win7-install.exe ../..')
        ...
        if flutter:
            ...
            system2('cargo build --locked --release --features ' + features)
            system2('git checkout src/ui/common.tis')
            system2('strip target/release/sehcontrol')
            system2('ln -s res/pacman_install && ln -s res/PKGBUILD')
            system2('HBB=`pwd` makepkg -f')
        system2('mv sehcontrol-%s-0-x86_64.pkg.tar.zst sehcontrol-%s-manjaro-arch.pkg.tar.zst' % (
            version, version))
        # pacman -U ./sehcontrol.pkg.tar.zst
        ...
        system2(
            'mv $HOME/rpmbuild/RPMS/x86_64/sehcontrol-%s-0.x86_64.rpm ./sehcontrol-%s-fedora28-centos8.rpm' % (
                version, version))
        # yum localinstall sehcontrol.rpm
        ...
        system2(
            'mv $HOME/rpmbuild/RPMS/x86_64/sehcontrol-%s-0.x86_64.rpm ./sehcontrol-%s-suse.rpm' % (
                version, version))
        # yum localinstall sehcontrol.rpm
    else:
        if flutter:
            if osx:
                ...
                # system2(
                #     'mv target/release/bundle/deb/sehcontrol*.deb ./flutter/sehcontrol.deb')
                build_flutter_deb(version, features)
        else:
            system2('cargo --locked bundle --release --features ' + features)
            if osx:
                system2(
                    'strip target/release/bundle/osx/Sehcontrol.app/Contents/MacOS/sehcontrol')
                system2(
                    'cp libsciter.dylib target/release/bundle/osx/Sehcontrol.app/Contents/MacOS/')
    ...
    #rcodesign sign --p12-file ~/.p12/sehcontrol-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./target/release/bundle/osx/Sehcontrol.app/Contents/MacOS/sehcontrol
    #rcodesign sign --p12-file ~/.p12/sehcontrol-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./target/release/bundle/osx/Sehcontrol.app/Contents/MacOS/libsciter.dylib
    #rcodesign sign --p12-file ~/.p12/sehcontrol-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./target/release/bundle/osx/Sehcontrol.app
    # goto "Keychain Access" -> "My Certificates" for below id which starts with "Developer ID Application:"
    codesign -s "Developer ID Application: {0}" --force --options runtime  ./target/release/bundle/osx/Sehcontrol.app/Contents/MacOS/*
    codesign -s "Developer ID Application: {0}" --force --options runtime  ./target/release/bundle/osx/Sehcontrol.app
    '''.format(pa))
                system2(
                    'create-dmg "Sehcontrol %s.dmg" "target/release/bundle/osx/Sehcontrol.app"' % version)
                os.rename('Sehcontrol %s.dmg' %
                          version, 'sehcontrol-%s.dmg' % version)
                if pa:
                    system2('''
    ...
    #rcodesign sign --p12-file ~/.p12/sehcontrol-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./sehcontrol-{1}.dmg
    codesign -s "Developer ID Application: {0}" --force --options runtime ./sehcontrol-{1}.dmg
    ...
    rcodesign notary-submit --api-key-path ../.p12/api-key.json  --staple sehcontrol-{1}.dmg
    # verify:  spctl -a -t exec -v /Applications/Sehcontrol.app
    '''.format(pa, version))
                else:
                    print('Not signed')
            else:
                # build deb package
                system2(
                    'mv target/release/bundle/deb/sehcontrol*.deb ./sehcontrol.deb')
                system2('dpkg-deb -R sehcontrol.deb tmpdeb')
                system2('mkdir -p tmpdeb/usr/share/sehcontrol/files/systemd/')
                system2('mkdir -p tmpdeb/usr/share/icons/hicolor/256x256/apps/')
                system2('mkdir -p tmpdeb/usr/share/icons/hicolor/scalable/apps/')
                system2(
                ...
                system2(
                    'cp res/sehcontrol-link.desktop tmpdeb/usr/share/applications/sehcontrol-link.desktop')
                os.system('mkdir -p tmpdeb/etc/sehcontrol/')
                os.system('cp -a res/startwm.sh tmpdeb/etc/sehcontrol/')
                os.system('mkdir -p tmpdeb/etc/X11/sehcontrol/')
                os.system('cp res/xorg.conf tmpdeb/etc/X11/sehcontrol/')
                os.system('cp -a DEBIAN/* tmpdeb/DEBIAN/')
                os.system('mkdir -p tmpdeb/etc/pam.d/')
                os.system('cp pam.d/sehcontrol.debian tmpdeb/etc/pam.d/sehcontrol')
                system2('strip tmpdeb/usr/bin/sehcontrol')
                system2('mkdir -p tmpdeb/usr/share/sehcontrol')
                system2('mv tmpdeb/usr/bin/sehcontrol tmpdeb/usr/share/sehcontrol/')
                system2('cp libsciter-gtk.so tmpdeb/usr/share/sehcontrol/')
                md5_file_folder("tmpdeb/")
                system2('dpkg-deb -b tmpdeb sehcontrol.deb; /bin/rm -rf tmpdeb/')
                os.rename('sehcontrol.deb', 'sehcontrol-%s.deb' % version)
