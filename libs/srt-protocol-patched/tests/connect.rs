...
fn complete(mut conn: Conn, start: Instant) -> (Connection, Connection) {
    /// **Sehcontrol patch — upstream budgets 20 s here.**
    ///
    /// This is simulated time, not wall clock, and these scenarios run at a 70%
    /// drop rate, so how long they take is dominated by how often the caller
    /// retransmits an unanswered handshake. Upstream retransmitted on every
    /// 100 ms tick; the caller now honours a 250 ms deadline tied to the packet
    /// (see `HANDSHAKE_RETRANSMIT_INTERVAL`), which is what libsrt does and what
    /// stops MediaMTX replacing its publisher mid-handshake.
    ///
    /// The budget is scaled by that same factor rather than the test being
    /// weakened: what it asserts — that a 70%-loss link still completes the
    /// handshake — is unchanged, and so are the seeds it pins.
