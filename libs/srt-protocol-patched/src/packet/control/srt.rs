...
bitflags! {
    ...
    pub struct SrtShakeFlags: u32 {
        ...
        /// **Not "supports stream ID"** — upstream's comment here is a guess (see
        /// the one below it). `SRT_OPT_STREAM` selects *file/stream (buffer)
        /// transmission mode* instead of live/message mode. The stream id
        /// travels as its own `SRT_CMD_SID` handshake extension and is not
        /// gated on this bit, so sending a sid without it is not a
        /// contradiction.
        ///
        /// Deliberately absent from `SUPPORTED`: this crate implements live
        /// mode only (`options`: "SRTO_MESSAGEAPI - only \"live\" is
        /// supported", and `uri.rs` refuses a `messageapi` parameter), and a
        /// peer compares this bit against its own mode — a mismatch is what
        /// `CoreRejectReason::MessageApi` (1012, "streamapi/messageapi
        /// collision") exists for. Setting it would claim a mode we do not
        /// implement.
        ///
        /// Measured, not assumed: adding it to `SUPPORTED` was tried against
        /// MediaMTX 1.9.3 and the answer did not change — still
        /// `Core(Rogue)`, 4/4 attempts, same latency. It is not the cause of
        /// that rejection.
        const STREAM = 0x40;
        ...
        /// **Sehcontrol patch — upstream declares `0x27` here.**
        ///
        /// `TLPKTDROP` and `NAKREPORT` were missing from upstream's list even
        /// though the crate implements both and enables both by default in live
        /// mode: `options::Sender::too_late_packet_drop` and
        /// `options::Receiver::{too_late_packet_drop, nak_report}` all default
        /// to `true`, and the periodic NAK is driven for real
        /// (`check_timers` → `check_nak` → `receiver::on_nak_event` →
        /// `ControlTypes::Nak`). So this corrects an under-declaration; it does
        /// not claim anything new.
        ///
        /// It matters because `datarhei/gosrt` v0.7.0 — which MediaMTX 1.9.3
        /// embeds — refuses an HSv5 conclusion with `REJ_ROGUE` ("not all
        /// required flags are set") unless `TSBPDSND | TSBPDRCV | TLPKTDROP |
        /// PERIODICNAK | REXMITFLG` are all present. `PERIODICNAK` is this
        /// crate's `NAKREPORT`.
        ///
        /// `STREAM` stays off deliberately (see its own note above), and
        /// `PACKET_FILTER` too: neither is implemented here.
...
mod tests {
    ...
    /// Pins the capability set this crate honestly implements, and records the
    /// one experiment that has already been run against it.
    ///
    /// `STREAM` (0x40) is *not* "supports stream ID": it selects file/stream
    /// transmission mode, which this crate does not implement (live only).
    /// Adding it was measured against MediaMTX 1.9.3 and changed nothing — the
    /// `Core(Rogue)` rejection was identical, 4/4 attempts — so it is neither a
    /// fix nor an honest claim. If a future change adds it, this test should
    /// fail and send the reader here first.
    #[test]
    fn supported_declares_only_what_this_crate_implements() {
        // gosrt v0.7.0 requires TSBPDSND|TSBPDRCV|TLPKTDROP|PERIODICNAK|REXMITFLG
        // and answers REJ_ROGUE otherwise; HAICRYPT is ours on top of that.
        assert_eq!(SrtShakeFlags::SUPPORTED.bits(), 0x3F);
        for flag in [
            ...
            assert!(SrtShakeFlags::SUPPORTED.contains(flag), "{flag:?}");
        }
        assert!(
            !SrtShakeFlags::SUPPORTED.contains(SrtShakeFlags::STREAM),
            "live/message mode only; see the constant's documentation"
        );
        assert!(!SrtShakeFlags::SUPPORTED.contains(SrtShakeFlags::PACKET_FILTER));
    }

    /// The two flags this patch adds are only honest if the crate really does
    /// enable both mechanisms by default in live mode. If a future change flips
    /// a default, this fails next to the declaration that depends on it.
    #[test]
    fn the_added_flags_match_the_live_mode_defaults() {
        let receiver = crate::options::Receiver::default();
        assert!(receiver.nak_report, "NAKREPORT is declared to the peer");
        assert!(
            receiver.too_late_packet_drop,
            "TLPKTDROP is declared to the peer"
        );
    }

    /// The flags survive a round trip unchanged, so what the peer reads is what
    /// `SUPPORTED` declares — including the bits that are deliberately off.
    #[test]
    fn the_declared_flags_survive_serialization() {
        let handshake = Packet::Control(ControlPacket {
            timestamp: TimeStamp::from_micros(1),
            dest_sockid: SocketId(1),
            control_type: ControlTypes::Srt(SrtControlPacket::HandshakeRequest(SrtHandshake {
                version: SrtVersion::CURRENT,
        ...
        let Packet::Control(ControlPacket {
            control_type: ControlTypes::Srt(SrtControlPacket::HandshakeRequest(shake)),
            ..
        }) = deserialized
        else {
            panic!("expected an SRT handshake request");
        };
        assert_eq!(shake.flags, SrtShakeFlags::SUPPORTED);
        assert!(!shake.flags.contains(SrtShakeFlags::STREAM));
        // And the version this fork exists to raise is on the wire with it.
