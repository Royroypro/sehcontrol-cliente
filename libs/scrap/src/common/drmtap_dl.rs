...
impl DrmtapLib {
    fn load() -> Option<Self> {
        // Absolute path FIRST: the deb bundles the .so privately under /usr/lib/sehcontrol and does NOT register that dir with ld.so.
        const INSTALLED: &str = "/usr/lib/sehcontrol/libdrmtap.so.0";
