...
fn main() {
    ...
    // `cfg!(windows)`/`cfg!(target_env = "msvc")` below reflect the *build
    // script's own* compile target, i.e. the host running cargo - not the
    // TARGET this invocation of libsodium-sys is actually being linked into.
    // That's correct as long as host == target, but breaks cross-compiling
    // from a Windows host to a non-Windows target (e.g. Android): the crate
    // unconditionally falls back to its bundled Windows/MSVC x64 prebuilt,
    // ignoring SODIUM_LIB_DIR entirely, and even find_libsodium_env() below
    // picks the MSVC "libsodium" name instead of "sodium". Patched to check
    // the real TARGET string instead, so a Windows host cross-compiling to
    // Android still honors SODIUM_LIB_DIR for the actual target.
    let target = env::var("TARGET").unwrap_or_default();
    let target_is_windows = target.contains("windows");
    let target_is_msvc = target.contains("msvc");
...
fn find_libsodium_env() {
    ...
    let name = if env::var("TARGET").unwrap_or_default().contains("msvc") {
        "libsodium"
    } else {
        "sodium"
    };
